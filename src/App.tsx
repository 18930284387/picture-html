...
const STORAGE_KEY = 'pixelhub_photos';
...
export default function App() {
  ...
  // Initialize photos from localStorage or use initialPhotos
      ...
      <div className={`min-h-screen ${bg} flex flex-col`}>
        ...
        <div className="flex-1 p-6 overflow-auto">
          <UploadForm
            ...
            onClearPreview={() => setUploadPreview('')}
            onTitleChange={setUploadTitle}
            onTagsChange={setUploadTags}
            onPrivacyChange={setUploadPrivacy}
            onUpload={handleUpload}
            onTagClick={(tag) => setUploadTags(prev => prev ? prev + ', ' + tag : tag)}
          />
        </div>
      </div>
    );
            ...
            <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
              {filteredPhotos.map((photo, idx) => (
                <PhotoCard
                  ...
                  photo={photo}
                  darkMode={darkMode}
                  isFollowing={isFollowing[photo.author] || false}
